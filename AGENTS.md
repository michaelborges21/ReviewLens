...
- LLM: **`gemma4:12b` local via Ollama** (ADR-004). **Nenhuma chave de API no projeto** (ADR-011) — sem custo por token e sem segredo de repositório. A camada `src/bri/llm/` segue intercambiável por desenho: se um dia houver provider comercial, não precisa ser a Anthropic. Obrigatório na extração: enum no prompt + JSON Schema no `format` + `think: false`.
